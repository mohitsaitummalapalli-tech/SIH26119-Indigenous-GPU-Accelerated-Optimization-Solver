...
int main() {
    ...
    // 3. Validate Repository Architecture Boundaries
    ...
    // Attempt to locate repo root relative to current working directory or executable
    ...
    std::cout << "  [INFO] Inspecting repository root at: " << fs::absolute(repo_root).string() << "\n";
    ...
    for (const auto& rel_path : expected_directories) {
        ...
        if (fs::is_directory(p)) {
            ...
            std::cerr << "  [WARN] Expected layer directory not yet present: " << rel_path << "\n";
            // We report warnings if running before all READMEs are written, or fail if missing during final check
        }
    }
